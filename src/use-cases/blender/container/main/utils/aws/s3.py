...
from ..job.extensions import split_large_file
...
def download_directory(bucket_name, directory_prefix, local_directory):
    logger.info(
        "Downloading Directory from S3 Bucket. Bucket: {}, Directory Prefix: {}, Local Directory: {}".format(
            bucket_name, directory_prefix, local_directory
        )
    )
    try:
        ...
        for key in objKeys:
            file_path = os.path.join(local_directory, key.split('/')[-1])
            download(bucket_name, key, file_path)
    except ClientError as e:
        logger.error(e)
        return None
